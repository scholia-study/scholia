...
- `packages/md_prose_to_struct` — shared **annotated-prose** parser (`--corpus kant1|kant3|hegel1|hegel2|hegel3|hobbes1|peirce1|plato1`; `--translation` for the single-layer English edition, `--single` for a corpus with one curated layer and no orthography pair — see ADR 0011): footnotes, margin notes (`` {{$m `…`}} `` — see ADR 0010), figures, separators, indented runs, dual page-marker systems (`{{{ }}}` primary + `{{ }}` secondary; hegel1's primary mixes Roman and Arabic 1807 pages in one series, its secondary is the GW 9 concordance; hegel2's single system spans three GW volumes with dotted `21.68` values; hobbes1's single system suffixes the twice-printed 1651 pages `247b`; peirce1's single system is venue-qualified `PSM 12:1` because the selection spans five periodicals whose volume numbers collide). Corpus config assembled from `common::kant{1,3}` / `common::hegel{1,2,3}` / `common::hobbes1` / `common::peirce1` / `common::plato1` (TOC tables, filename rules, `meta` book/ref-system data). Corpus knobs: `source_splitter_en` (English source book) and `strong_colon_splits` (Early Modern period-strength colons), both hobbes1; `paren_protected_splits` (no sentence boundary inside or directly before a parenthesis — citation apparatus stays inside its host sentence), hegel2 + hegel3; `greek_splitter` + `single_layer_source` (Burnet's constituted Greek text has no orthography companion; `--translation` still builds the parity-locked English), plato1. Absorbed `kant{1,3}_md_to_struct` + `kant{1,3}_md_translation_to_struct` (2026-07).
- `packages/hegel1_tei_to_md` — DTA TEI → both curated German layers (`md_reviewed` diplomatic; `--layer modernized` + the checked-in decision/ops tables). Serves hegel1 AND hegel3 (`--page-key page_1812` — same `att.linguistic` TEI shape; footnotes, TeX fractions, gaps, slug dedup, `--promote-head` re-levelling added for the 1812). Gates: `scripts/hegel1_verify.sh`, `scripts/hegel1_verify_modernized.sh --regen`.
- `packages/hegel2_gw_to_md` — Digitale-Hegel-Edition JSON (GW 21/11/12) → hegel2's curated layers, steered by three curated tables in `assets/hegel2/curated/` (witness-derived `page_joins.tsv`, adjudicated `gw_dta_rulings.tsv`, composed `modernize_rulings.tsv`), refuse-to-emit on any unruled mismatch. Gate: `scripts/hegel2_verify.sh` (structure + DTA/Zeno witness alignment).
- `packages/hobbes1_tei_to_md` — EEBO-TCP A43998 TEI → hobbes1 `md_reviewed` (diplomatic 1651 Leviathan): margin notes → `` {{$m `…`}} `` tokens, pb → `{{{ N }}}` with `b`-suffixed duplicate pages, gaps kept as TCP display forms, in-crate ruling tables (`LABEL_RULINGS`, `FORCED_SPLIT_RULINGS`, front-div exclusions). `md_modernized` was bootstrapped from it and is hand-curated (provenance: `assets/hobbes1/curated/MODERNIZATION_NOTES.md`) — regenerating md_reviewed is safe, md_modernized is NOT regenerable.
- `packages/plato1_tei_to_md` — Perseus EpiDoc TEI (`tlg0059.tlg030.perseus-grc2.xml`, Burnet's Greek *Republic*) → plato1 `md_modernized` (the sole Greek layer; Stephanus `{{{ }}}` markers, `⟨ ⟩` / `[ ]` editorial brackets, Greek normalization via `common::greek`). Run-once bootstrap: after it runs the curated MD is the editorial surface and it is not run again.
- `packages/plato2_tei_to_md` — the same for `tlg0059.tlg023.perseus-grc2.xml`, Burnet's Greek *Gorgias* → plato2 `md_modernized`. Written separately rather than generalised (the *Republic* is narrated and rebuilds paragraphing from 4,234 `ed="P"` hints; the *Gorgias* is staged and takes its block structure from 1,081 speech turns). Also run-once. Note `<said rend="merge">` continues the preceding speech and must not emit a second speaker label, and a turn's opening Stephanus milestone sits *inside* `<said>`, before `<label>`.
...
- `packages/struct_to_db` — **the one importer** for every `text_struct` JSON (Kant, poetry, drama): reconcile-in-place (carries sentence UUIDs + anchored quotations across edits), translation mode (`--source-book-slug`, 1:1-locked to a source book, footnote-aware), footnotes, sub-work sources. `--allow-insertion` unlocks mid-book node insertion for growing selections (peirce1's ingest.sh arm only — ADR 0012); default stays strictly append-only. Genre-agnostic (`block_type` is a pass-through string). Absorbed `kant1_struct_to_db`/`kant3_struct_to_db` (2026-07); formerly `poetry_struct_to_db`.
...
- `packages/md_drama_to_struct` — shared **drama** parser (`--corpus ibsen1|plato2`, or `--translation` for the single-layer English edition), tokenising the `@ speaker` / `@stage` / `| verse` / `{{{ N }}}` markup into the `text_struct` schema, also imported by the reused `struct_to_db`. Canonical TOCs in `common::ibsen1` / `common::plato2`, both using the shared node shape `common::drama::Node`; `just struct <corpus>` builds both editions → `just db <corpus>` imports source then the English translation (`struct_to_db --source-book-slug`). See ADR 0005. Drama dialogue splits with `common::sentences::split_sentences_structural` (layer-consistent), not `split_sentences_en` — **except** under the `greek_splitter` corpus flag (plato2), which routes to `split_sentences_grc` and disables stage-direction peeling. Corpus knobs: `greek_splitter` and `subpage_letter_sort` (Stephanus `447a` sorts as page×10+letter; parsing it as an integer yields 0 for every marker), both plato2. Footnotes (`[^n]` / `[^n]: text`) are supported, and their text is split with the *dialogue's* splitter — so on a Greek edition an abbreviated English citation (`Pind. fr. 169`) silently becomes two footnote sentences; expanded form is mandatory.
