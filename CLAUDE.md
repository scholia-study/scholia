...
- `packages/md_prose_to_struct` — shared **annotated-prose** parser (`--corpus kant1|kant3|hegel1|hobbes1`, `--translation` for the single-layer English edition): footnotes, margin notes (`` {{$m `…`}} `` — see ADR 0010), figures, separators, indented runs, dual page-marker systems (`{{{ }}}` primary + `{{ }}` secondary; hegel1's primary mixes Roman and Arabic 1807 pages in one series, its secondary is the GW 9 concordance; hobbes1's single system suffixes the twice-printed 1651 pages `247b`). Corpus config assembled from `common::kant{1,3}` / `common::hegel1` / `common::hobbes1` (TOC tables, filename rules, `meta` book/ref-system data). Corpus knobs: `source_splitter_en` (English source book) and `strong_colon_splits` (Early Modern period-strength colons), both hobbes1. Absorbed `kant{1,3}_md_to_struct` + `kant{1,3}_md_translation_to_struct` (2026-07).
- `packages/hegel1_tei_to_md` — DTA TEI → both hegel1 curated layers: `md_reviewed` (diplomatic) and, via `--layer modernized` + the checked-in decision/ops tables in `assets/hegel1/curated/`, `md_modernized`. Gates: `scripts/hegel1_verify.sh`, `scripts/hegel1_verify_modernized.sh --regen`.
