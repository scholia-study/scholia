...
- `packages/md_prose_to_struct` — shared **annotated-prose** parser (`--corpus kant1|kant3|hegel1`, `--translation` for the single-layer English edition): footnotes, figures, separators, indented runs, dual page-marker systems (`{{{ }}}` primary + `{{ }}` secondary; hegel1's primary mixes Roman and Arabic 1807 pages in one series, its secondary is the GW 9 concordance). Corpus config assembled from `common::kant{1,3}` / `common::hegel1` (TOC tables, filename rules, `meta` book/ref-system data). Absorbed `kant{1,3}_md_to_struct` + `kant{1,3}_md_translation_to_struct` (2026-07).
