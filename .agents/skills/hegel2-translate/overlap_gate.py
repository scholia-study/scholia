#!/usr/bin/env python3
"""Translation-independence gate for hegel2 English files (ported from hegel1).
...
(every `*.txt` in `assets/hegel2/control/`: miller, digiovanni) —
from-memory leakage can converge on any one of them individually. The
miller control on hand is an abridged text; digiovanni is complete. PASS requires overlap < 3% AND longest run < 15 words
...
CONTROL_DIR = REPO / "assets/hegel2/control"
