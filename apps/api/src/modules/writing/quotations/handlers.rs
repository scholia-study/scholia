...
pub async fn list_quotations(
    ...
    let quotations = crate::modules::writing::quotations::db::list_quotations_for_node(
        &state.pool,
        user.id,
        book_id,
        node_id,
...
pub async fn create_quotation(
    ...
    let (quotation, created) = crate::modules::writing::quotations::db::create_quotation(
        &state.pool,
        user.id,
        book_id,
        body.sentence_start,
...
pub async fn delete_quotation(
    ...
    crate::modules::writing::quotations::db::delete_quotation(&state.pool, quotation_id, user.id)
        .await?;
...
pub async fn list_notes(
    ...
    // Verify ownership
    let (owner_id, _) =
        crate::modules::writing::quotations::db::get_quotation_owner(&state.pool, quotation_id)
            .await?;
    if owner_id != user.id {
    ...
    let notes =
        crate::modules::writing::quotations::db::list_notes(&state.pool, quotation_id).await?;
...
pub async fn create_note(
    ...
    let current =
        crate::modules::writing::quotations::db::get_user_note_count(&state.pool, user.id).await?;
    let max = crate::modules::writing::quotations::db::get_note_limit(&user.roles);
    if current >= max as i64 {
        return Err(AppError::BadRequest(format!(
    ...
    let note = crate::modules::writing::quotations::db::create_note(
        &state.pool,
        user.id,
        quotation_id,
        &body.body,
...
pub async fn update_note(
    ...
    crate::modules::writing::quotations::db::update_note(
        &state.pool,
        note_id,
        user.id,
        body.body.as_deref(),
...
pub async fn delete_note(
    ...
    crate::modules::writing::quotations::db::delete_note(&state.pool, note_id, user.id).await?;
...
pub async fn list_tags(
    ...
    let tags = crate::modules::writing::quotations::db::list_tags(&state.pool, user.id).await?;
...
pub async fn list_all_quotations(
    ...
    if include_books {
        let book_quotations = crate::modules::writing::quotations::db::list_all_quotations(
            &state.pool,
            user.id,
        ...
        for q in book_quotations {
            quotations.push(UnifiedQuotationResponse::Book {
                ...
                anchor_sentence_start_id: q.anchor_sentence_start_id,
                anchor_sentence_end_id: q.anchor_sentence_end_id,
                anchor_block_type: q.anchor_block_type,
    ...
    if include_articles && params.book_slug.is_none() {
        let article_quotations =
            crate::modules::writing::article_quotations::db::list_article_quotations_for_unified(
                &state.pool,
                user.id,
            )
            .await?;
    ...
    let limits = crate::modules::writing::quotations::db::get_quotation_limits_response(
        &state.pool,
        user.id,
        &user.roles,
    )
...
pub async fn list_all_notes(
    ...
    let notes = crate::modules::writing::quotations::db::list_all_notes(
        &state.pool,
        user.id,
        params.book_slug.as_deref(),
    )
    ...
    let limits = crate::modules::writing::quotations::db::get_note_limits_response(
        &state.pool,
        user.id,
        &user.roles,
    )
