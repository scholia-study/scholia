...
use crate::modules::identity::accounts::models::{PublicProfileResponse, UserHandleResponse};
use crate::modules::writing::PublicArticleListQuery;
use crate::system::auth::handle::validate_handle;
...
pub async fn get_public_profile(
    ...
    let row =
        crate::modules::identity::accounts::db::get_public_profile_by_handle(&state.pool, &handle)
            .await?;

    let roles =
        crate::modules::identity::accounts::db::list_role_names(&state.pool, row.id).await?;
    let public_roles = crate::system::auth::permissions::filter_public_roles(&roles);
    ...
    let per_page = params
        ...
        .clamp(1, 100);
    let (articles, article_total) = crate::modules::writing::list_published_articles_by_author(
        &state.pool,
        row.id,
        page,
        per_page,
...
pub async fn get_handle_by_id(
    ...
    let handle =
        crate::modules::identity::accounts::db::get_handle_by_id(&state.pool, user_id).await?;
