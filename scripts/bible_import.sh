#!/usr/bin/env bash
#
# Import all five public-domain English Bibles — the shared run logic for
# local dev (scripts/db_bible.sh) and the ingest-bible job image.
#
# KJV runs first because it's the canonical translation (its verse counts
# seed the parity guard) and DARBY's alignment seeder reads KJV's
# page_markers; the other four are independent and import in parallel.
#
# The job image sets BIBLE_TO_DB_BIN (binary baked in); locally the importer
# is cargo-built.
set -euo pipefail

if [ -n "${BIBLE_TO_DB_BIN:-}" ]; then
    BIN="$BIBLE_TO_DB_BIN"
else
