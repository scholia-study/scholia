    ...
    <div className="flex w-64 shrink-0 border-r border-stone-200">
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-stone-200 bg-white shrink-0">
          <span className="text-sm text-stone-500 flex-1">Select a book</span>
          {onClose && (
            <button
              onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {isLoading && <p className="text-stone-400 text-sm p-2">Loading...</p>}
          {error ? <p className="text-red-500 text-sm p-2">Failed to load books.</p> : null}
          {books && (
            <ul className="space-y-1">
              {books.map((book) => (
                <li key={book.id}>
                  ...
                  >
                    <div className="text-sm text-stone-900">{book.title}</div>
