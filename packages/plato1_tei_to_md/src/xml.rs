...
mod tests {
    ...
    fn parses_flat_text_and_element() {
        let nodes = parse_body("<body>κατέβην <add>χθὲς</add> εἰς.</body>").unwrap();
        assert_eq!(nodes.len(), 3);
        assert_eq!(nodes[0], Node::Text("κατέβην ".to_string()));
        match &nodes[1] {
            Node::Element { name, children, .. } => {
                assert_eq!(name, "add");
                assert_eq!(children, &[Node::Text("χθὲς".to_string())]);
            }
    ...
    fn parses_self_closing_with_attrs() {
        let nodes =
            parse_body(r#"<body><milestone unit="section" resp="Stephanus" n="327a"/></body>"#)
                .unwrap();
        match &nodes[0] {
            Node::Element { name, attrs, .. } => {
                assert_eq!(name, "milestone");
                assert_eq!(attrs.get("unit").map(String::as_str), Some("section"));
                assert_eq!(attrs.get("n").map(String::as_str), Some("327a"));
            }
            _ => panic!("expected element"),
        }
    }
