...
use crate::model::*;
use text_struct::parse::{FrontMatter, parse_front_matter, scan_md_files, strip_indent};
...
pub fn build(corpus: &Corpus) -> Result<Output, Err> {
    ...
    let modernized_files: HashSet<String> = scan_md_files(modernized_dir)
        ...
        .into_iter()
        .collect();
    let reviewed_files: HashSet<String> = scan_md_files(reviewed_dir)
        .map_err(|e| format!("cannot read {}: {e}", corpus.reviewed_dir))?
        .into_iter()
        .collect();
