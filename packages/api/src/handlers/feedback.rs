...
pub async fn create_feedback(
    ...
    // defence — auth is.
    let recent = db::feedback::count_feedback_last_24h(&state.pool, user.id).await?;
    ...
    let feedback = db::feedback::create_feedback(
        &state.pool,
        user.id,
        trimmed,
        body.url.as_deref(),
...
pub async fn list_feedback(
    ...
    let list = db::feedback::list_feedback(&state.pool, &statuses, page, per_page).await?;
    Ok(Json(list))
}
...
pub async fn get_feedback(
    ...
    let f = db::feedback::get_feedback(&state.pool, id).await?;
    Ok(Json(f))
}
...
pub async fn update_feedback(
    ...
    let f = db::feedback::update_feedback(
        &state.pool,
        id,
        user.id,
        body.status,
